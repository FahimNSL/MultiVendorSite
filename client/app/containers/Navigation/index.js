...
class Navigation extends React.PureComponent {
  ...
  //   );
  // }
  // Update the renderSuggestion method in client
  renderSuggestion(suggestion, { query, isHighlighted }) {
    const BoldName = (suggestion, query) => {
      const matches = AutosuggestHighlightMatch(suggestion.name, query);
      const parts = AutosuggestHighlightParse(suggestion.name, matches);
    ...
    return (
      <Link to={`/${suggestion.type.toLowerCase()}/${suggestion.slug}`}>
        <div className='d-flex'>
          {/* Adjust the rendering based on the suggestion type (Product, Brand, or Category) */}
          ...
          <Row className='align-items-center top-header'>
            ...
            >
              <div className='brand'>
                ...
                <Link to='/'>
                  <h1 className='logo'>NSL Store</h1>
                </Link>
              </div>
            </Col>
            <Col
            ...
            >
              <Autosuggest
                ...
                //   history.push(`/shop/category/${item.suggestion.slug}`);
                // }}

                onSuggestionSelected={(_, item) => {
                  const { type, slug } = item.suggestion;
                  let path;
