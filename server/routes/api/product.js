...
// Add a new route to handle searching across products, brands, and categories
router.get('/search/:query', async (req, res) => {
  try {
    const query = req.params.query;

    // Search for products, brands, and categories based on the query
    const productResults = await Product.find({ name: { $regex: new RegExp(query), $options: 'i' }, isActive: true });
    const brandResults = await Brand.find({ name: { $regex: new RegExp(query), $options: 'i' }, isActive: true });
    const categoryResults = await Category.find({ name: { $regex: new RegExp(query), $options: 'i' }, isActive: true });
...
router.post(
  ...
  auth,
  // (req, res, next) => {
  //   console.log("User Info Before Role Check:", req.user);
  //   next();
  // },
  ...
  async (req, res) => {
    ...
    try {
      ...
      const image = req.file; // Array of uploaded images
      console.log(image);


      // Map uploaded images to get their URLs and keys
      // const imageUrl = "";
      // const imageKey = "";
      // for (const image of images) {
      const { imageUrl, imageKey } = await cloudinaryUpload(image); // Assuming you're using S3Upload function
      // imageUrl = imageUrl
      // imageKey = imageKEY
      // }
